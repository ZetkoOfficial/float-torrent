...
impl OperationSequenceProvider {
    fn combine(&self, length: usize, sequences: &[Vec<f64>]) -> Result<Vec<f64>> {
        if !sequences.iter().all(|s| s.len() == length) { 
            Err(Error::sequence_arithmetic_error("Prodobljene dolžine zaporedij se ne ujemajo"))
        } else {
            let mut result = vec![];
...
impl OperationSequence for SumSequence {
    ...
    fn get_info(&self) -> sequence_provide::SequenceInfo {
        sequence_provide::SequenceInfo {
            name: "sum".to_owned(),
            description: "Zaporedje s členi enakimi vsoti členov podanih dveh zaporedij".to_owned(),
            parameters: 0,
            sequences: 2
        }
    }
...
impl OperationSequence for ProductSequence {
    ...
    fn get_info(&self) -> sequence_provide::SequenceInfo {
        sequence_provide::SequenceInfo {
            name: "prod".to_owned(),
            description: "Zaporedje s členi enakimi produktu členov podanih dveh zaporedij".to_owned(),
            parameters: 0,
            sequences: 2
        }
    }
